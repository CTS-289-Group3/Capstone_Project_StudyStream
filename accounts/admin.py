...
class AssignmentAdmin(admin.ModelAdmin):
    list_display  = ['title', 'course', 'user', 'due_date', 'status', 'priority_level']
    list_filter   = ['status', 'priority_level', 'course__semester', 'user']
    inlines       = [SubtaskInline]
    filter_horizontal = ['tags']

@admin.register(AssignmentSubtask)
class SubtaskAdmin(admin.ModelAdmin):
    list_display = ['title', 'assignment', 'status', 'step_order']
